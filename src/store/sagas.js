...
function* get_games_listWorker(action) {
  try {
    const result = yield call(apiService.get_games_list, action)
    yield put(actions.get_games_listSucceeded(result))
  } catch (err) {
    yield put(actions.get_games_listFailed(err))
  }
}
function* get_games_listWatcher() {
  yield takeEvery(types.GET_GAMES_LIST, get_games_listWorker)
}
function* get_games_readWorker(action) {
  try {
    const result = yield call(apiService.get_games_read, action)
    yield put(actions.get_games_readSucceeded(result))
  } catch (err) {
    yield put(actions.get_games_readFailed(err))
  }
}
function* get_games_readWatcher() {
  yield takeEvery(types.GET_GAMES_READ, get_games_readWorker)
}
...
export default function* rootSaga() {
  const sagas = [
    get_games_listWatcher,
    get_games_readWatcher,
